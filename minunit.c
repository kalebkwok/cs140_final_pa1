...
/*--------------------------------------------------------------------
 ...
 Purpose:
  ...
  ing this run, variable _mu_tests_run increments by one.
  iable _mu_tests_failed increments by one if this test fails.

 In arg:
  test_fun: a pointer to a function which returns a string.

 This function has no argument and should use mu_assert to check an error
 and return a message. Return: If the test fails, the function should
 return a string describing the failing test.
